...
- To set a proposal's Scope of Work or Highlights, use update_estimate's `scope` and `highlights` fields — never write scope or highlights into project_name. Always confirm WHICH estimate (by number and client) before writing, so content doesn't land on the wrong project.
